 ...
 */
public class JsvapiSlice

{
	private final static Logger logger = Logger.getLogger(JsvapiSlice.class);
...
//			ic = Ice.Util.initialize(args);
			...
			if(BaseTools.getConfigString("sliceserverhost").equals(""))
			{
				base = ic.stringToProxy("scasvapimethod:default -p 10000 -h 127.0.0.1");
			}
			else
			{
				base = ic.stringToProxy("scasvapimethod:default -p 10000 -h " + BaseTools.getConfigString("sliceserverhost"));
			}
...
//		}
	...
	public static JsvapiSlice getInstance(String webDir) 
	{
		if(admin==null)
		{
			logger.info("执行Jsvapi初 始化......;webDir=" + webDir);
			JsvapiSlice jsvapi=new JsvapiSlice();
			...
			try
			{
				...
				prop.load(is);
				JsvapiSlice.admin = new GeneralCacheAdministrator(prop);
			} catch (Exception ex)
			{
				ex.printStackTrace();
			}finally{
		...
		return new JsvapiSlice();
	}
	public static JsvapiSlice getInstance()
	{
...
//		DisplayUtilMapInMap(fmap);
	...
	public void test2()
	{
		JsvapiSlice svapi = new JsvapiSlice();
		Map<String, Map<String, String>> fmap = new FastMap<String, Map<String, String>>();
		Map<String, String> ndata = new FastMap<String, String>();
		StringBuilder estr = new StringBuilder();
